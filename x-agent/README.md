...
| `mcp/` | The MCP server: 16 tools over stdio. TypeScript, `@modelcontextprotocol/sdk`, Playwright, zod, adm-zip. Nothing else. |
...
confirm the server with `/mcp` — you should see 16 tools.
...
The extend-tier tools (`wp_tokens_apply`, `wp_block_install`, `wp_snapshot`) refuse with
...
npm run list-tools       # prints all 16 tools with their full input schemas
npm run build
```
