...
export function buildImagePrompt(intent, style) {
...
export function buildSurfacePrompt(entry, style) {
    ...
    if (entry.class === 'spot' && entry.ground_baked) {
        if (entry.hexes[0])
            parts.push(`on a solid uniform background of exactly ${entry.hexes[0]}`);
    }
...
export class GeminiImages {
    ...
    async generate(prompt, aspectRatio = '16:9') {
        ...
        throw new Error(`Gemini image call failed: ${lastErr?.message ?? String(lastErr)}`);
    }
}
