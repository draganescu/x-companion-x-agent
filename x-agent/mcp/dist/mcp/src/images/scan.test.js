...
import { buildImagePrompt } from './gemini.js';
import { applyImage, findPlaceholders, nodeAt, toApiAspect } from './scan.js';
...
describe('applyImage', () => {
    it('swaps url and id on the exact node, keeping the intent', () => {
        ...
        expect(node.attributes.metadata.imageIntent).toBe('The red windmill at dusk.');
    });
    it('refuses a node whose url changed since the scan', () => {
        const blocks = tree();
        const [ref] = findPlaceholders(blocks);
        const node = nodeAt(blocks, ref.path);
        node.attributes.url = 'http://x/someone-elses-photo.jpg';
        expect(applyImage(blocks, ref, { id: 42, url: 'http://x/real.jpg' })).toBe(false);
        expect(node.attributes.url).toBe('http://x/someone-elses-photo.jpg');
    });
});
