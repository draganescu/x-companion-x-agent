import { z } from 'zod';
import { MANIFEST_SECTIONS, ManifestSchema } from '../schemas.js';
...
const InputSchema = z.object({
    ...
    section: z
        ...
        .optional()
        .describe('Return one interfaces-v2 section instead of the blocks map: styles | variations | global_styles | bindings | data_model | features. Mirrors GET /manifest?section=.'),
    client_capture: z
        .boolean()
        .optional()
        .describe('Merge client-registered variations and styles captured from the warm harness page (source: "client"). Default true on an interfaces-v2 instance; the capture is cached per fingerprint. Pass false to skip warming the browser.'),
});
...
const OutputSchema = ManifestSchema.omit({ blocks: true })
    ...
    .catchall(z.unknown());
export function applyManifestView(manifest, opts) {
    ...
    for (const [name, block] of Object.entries(manifest.blocks)) {
        if (prefix && !name.startsWith(prefix))
            continue;
        if (dynamicOnly && !block.is_dynamic)
            continue;
...
function summarise(block) {
    const s = { title: block.title, is_dynamic: block.is_dynamic };
    if (block.parent !== undefined)
        s.parent = block.parent;
    if (block.ancestor !== undefined)
        s.ancestor = block.ancestor;
    return s;
}
/**
 ...
 */
export function mergeClientCapture(manifest, capture) {
    const blocks = {};
    for (const [name, block] of Object.entries(manifest.blocks)) {
        const merged = { ...block };
        ...
        if (clientVars.length) {
            ...
            const fresh = clientVars
                ...
                .map((v) => ({ ...v, source: 'client' }));
            if (fresh.length)
                merged.variations = [...server, ...fresh].sort((a, b) => a.name.localeCompare(b.name));
        }
        ...
        if (clientStyles.length) {
            ...
            const fresh = clientStyles
                ...
                .map((s) => ({ ...s, source: 'plugin' }));
            if (fresh.length)
                merged.styles = [...server, ...fresh].sort((a, b) => a.name.localeCompare(b.name));
        }
        blocks[name] = merged;
    }
...
export function sectionEnvelope(manifest, section) {
    const envelope = {
...
export const wpManifest = defineTool({
    ...
    handler: async (input, ctx) => {
        ...
        let captureInfo = { performed: false };
        if (captureWanted && v2 && wantsBlocks) {
            try {
                ...
                captureInfo = { performed: false, error: e.message };
            }
        }
        if (args.section) {
            return {
        ...
        if (args.summary !== undefined)
            viewOpts.summary = args.summary;
        if (args.filter !== undefined)
            viewOpts.filter = args.filter;
