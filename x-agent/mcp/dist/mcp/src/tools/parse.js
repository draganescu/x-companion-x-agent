 ...
 */
export function stripToTree(parsed) {
    let dropped = 0;
    const walk = (nodes) => {
        const out = [];
        for (const n of nodes ?? []) {
            ...
            const node = { name: n.blockName };
            if (n.attrs && typeof n.attrs === 'object' && Object.keys(n.attrs).length > 0)
                node.attributes = n.attrs;
...
export const wpParse = defineTool({
    ...
    description: 'POST /parse. Brownfield lifting: turns existing post_content into a TreeIR you can edit and recompile. Returns both the verbatim parse_blocks() output (with innerHTML, for inspection) and the stripped TreeIR (no innerHTML — a tree never carries compiler output).',
