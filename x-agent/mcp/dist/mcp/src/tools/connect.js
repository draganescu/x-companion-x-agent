...
export const wpConnect = defineTool({
    ...
    description: 'Resolve connection config (tool args > .x-agent.json > X_WP_* env), probe GET /fingerprint and GET /manifest, and cache the manifest keyed by fingerprint. Refuses plain http:// unless the host is localhost/127.0.0.1/[::1]/*.localhost/a playground host. The returned fingerprint is the epoch every TreeIR must carry.',
...
export const wpDisconnect = defineTool({
    ...
    description: 'Close the warm Playwright harness session (if any), drop the block factory, clear the manifest/pattern/fingerprint caches and forget the pinned connection overrides. Use before switching instances or when the instance was rebuilt underneath the agent.',
