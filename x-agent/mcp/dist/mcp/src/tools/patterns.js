...
export const wpPatterns = defineTool({
    ...
    description: 'The retrieval corpus of layout idioms actually registered on the instance. Check this BEFORE inventing a novel composition: adapting a registered pattern is the cheapest correct answer. Fetched once per fingerprint, filtered locally by substring and pattern category.',
    ...
    handler: async (input, ctx) => {
        ...
        const matched = all.filter((p) => {
            if (args.category && !(p.categories ?? []).includes(args.category))
                return false;
            if (!q)
                return true;
