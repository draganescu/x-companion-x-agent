...
const AttributeSchema = z.object({
    ...
    control: z
        ...
        .optional()
        .describe('Inspector control. Omit to infer from type (array/object then scaffold a raw-JSON fallback you MUST replace with a purpose-built control before install). "textarea" on an array edits it as one item per line.'),
    ...
    handler: async (input, ctx) => {
        ...
        if (args.interactivity === 'interactivity-api') {
            ...
            try {
                ...
                const feature = manifest.features?.interactivity_api;
                available = feature?.available;
            }
            catch (e) {
                throw new XError('invalid_input', `interactivity:"interactivity-api" needs a connected instance to verify features.interactivity_api, and none is reachable: ${e.message}`, 'Connect an instance (wp_connect) or use interactivity:"view-script" — the vanilla rung needs no platform feature.');
            }
            if (available === false) {
                throw new XError('invalid_input', 'The target instance does not provide the Interactivity API (manifest features.interactivity_api.available = false).', 'Use interactivity:"view-script" — the vanilla rung works everywhere. The Interactivity API rung requires WordPress 6.5+.');
            }
        }
        ...
        if (args.attributes)
            scaffoldArgs.attributes = args.attributes;
        if (args.dir)
            scaffoldArgs.dir = args.dir;
        if (args.description)
            scaffoldArgs.description = args.description;
        if (args.version)
            scaffoldArgs.version = args.version;
        if (args.force)
            scaffoldArgs.force = true;
        if (args.interactivity)
            scaffoldArgs.interactivity = args.interactivity;
        if (args.stylesheet)
            scaffoldArgs.stylesheet = true;
        return factory.scaffold(scaffoldArgs);
    },
});
export const tools = [wpBlockScaffold];
export default wpBlockScaffold;
//# sourceMappingURL=blockScaffold.js.map