        ...
        if (publicTypes.length) {
            try {
                ...
                for (const p of publicTypes) {
                    ...
                    if (taken.has(claim)) {
                        warnings.push(`post type "${p.slug}" claims /${claim}/ but the site already serves a page or post at that slug — ` +
                            `its permalinks${p.has_archive ? ' and archive' : ''} will fight it. Pick a different rewrite_slug now.`);
                    }
                }
            }
            catch {
                warnings.push('Could not check the URL map against an instance (none reachable): collisions between a public post type and existing page slugs will only surface after publish.');
            }
        }
...
//# sourceMappingURL=schemaScaffold.js.map