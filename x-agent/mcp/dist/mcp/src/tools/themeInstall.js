...
export const wpThemeInstall = defineTool({
    ...
    description: 'POST /themes/install with the zip from wp_theme_build_test, activate it, then refresh the manifest and reload the harness session onto the new epoch. Activation moves the epoch: the fingerprint returned is the steady-state one the refreshed manifest serves. EXTEND TIER: refused with {code:"posture_forbidden"} on production posture. The theme is DELIVERABLE — named, versioned, deletable from wp-admin; nothing ever removes it at run end.',
    ...
    handler: async (input, ctx) => {
        ...
        if (!fs.existsSync(zipPath) || !fs.statSync(zipPath).isFile()) {
            throw errInvalidInput(`No package at ${zipPath}.`, 'Pass the zip_path returned by wp_theme_build_test. If that tool returned no zip_path, the theme failed its gate and its ThemeSpec must be repaired first.', { zip_path: args.zip_path });
        }
        const inspection = inspectThemePackage(zipPath);
        if (!inspection.ok) {
            throw new XError('invalid_input', `The theme package at ${zipPath} violates the install policy: ${inspection.reasons.join('; ')}.`, 'Re-run wp_theme_build_test rather than hand-editing a zip.', { zip_path: zipPath, reasons: inspection.reasons });
        }
        ...
        if (fp.posture === 'production')
            throw errPostureForbidden('/themes/install');
        ...
        try {
            ...
            live.logger.warn(`installed theme ${res.installed.slug} but could not refresh the manifest: ${e.message}`);
        }
        ...
        if (live.session !== undefined) {
            try {
                ...
                live.logger.warn(`installed theme ${res.installed.slug} but the harness session did not reload: ${e.message}`);
            }
        }
        const out = {
        ...
        if (res.previous_theme)
            out.previous_theme = res.previous_theme;
        return out;
    },
});
export const tools = [wpThemeInstall];
export default wpThemeInstall;
//# sourceMappingURL=themeInstall.js.map