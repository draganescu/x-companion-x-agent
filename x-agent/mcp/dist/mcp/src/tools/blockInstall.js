...
export const wpBlockInstall = defineTool({
    ...
    handler: async (input, ctx) => {
        ...
        if (!fs.existsSync(zipPath) || !fs.statSync(zipPath).isFile()) {
            throw errInvalidInput(`No package at ${zipPath}.`, 'Pass the zip_path returned by wp_block_build_test. If that tool returned no zip_path, the block failed its smoke test and must be fixed first.', { zip_path: args.zip_path });
        }
        ...
        if (!inspection.ok) {
            throw new XError('invalid_input', `The package at ${zipPath} violates the install policy: ${inspection.reasons.join('; ')}.`, 'CONTRACT.md §5 pins the policy. Re-run wp_block_build_test rather than hand-editing a zip.', { zip_path: zipPath, reasons: inspection.reasons });
        }
        ...
        if (fp.posture === 'production')
            throw errPostureForbidden('/blocks/install');
        ...
        try {
            ...
            live.logger.warn(`installed ${res.installed.name} but could not refresh the manifest: ${e.message}`);
        }
        ...
        if (live.session !== undefined) {
            try {
                ...
                live.logger.warn(`installed ${res.installed.name} but the harness session did not reload: ${e.message}`);
            }
        }
        return {
...
//# sourceMappingURL=blockInstall.js.map