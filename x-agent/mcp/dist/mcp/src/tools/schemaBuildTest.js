...
export const wpSchemaBuildTest = defineTool({
    ...
    description: 'THE SAFETY GATE for schema packages, mirroring wp_block_build_test. Static policy scan first (no $wpdb, no eval/exec — core APIs only), then a throwaway Playground boots the package and asserts: every declared post type in /wp/v2/types, every meta key REST-visible with a schema, taxonomies registered, every route answering as declared (2xx for a valid nonce\'d call, 401/403 unauthenticated on protected routes), binding sources resolvable, and a clean uninstall. Any failure returns structured detail and NO zip — nothing reaches an instance that has not passed here.',
    ...
    handler: async (input, ctx) => {
        ...
        const out = { built: result.built, smoke: result.smoke };
        if (result.zip_path)
            out.zip_path = result.zip_path;
        if (result.build_log)
            out.build_log = result.build_log;
        return out;
    },
});
export const tools = [wpSchemaBuildTest];
//# sourceMappingURL=schemaBuildTest.js.map