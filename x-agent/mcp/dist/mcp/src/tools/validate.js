import { z } from 'zod';
import { TreeIRSchema, DiagnosticsSchema, checkWithZod } from '../schemas.js';
 ...
 */
export function localTreeSchemaPrecheck(tree) {
    ...
    return {
        ...
        diagnostics: issues.map((i) => ({
            ...
            path: i.path,
            message: i.message,
            fix_hint: 'Fix the tree against schemas/tree-ir.schema.json. BlockNode allows only {name, attributes, innerBlocks}; innerHTML is a compiler output and is a hard schema error here.',
        })),
    };
}
export const wpValidate = defineTool({
    ...
    handler: async (input, ctx) => {
        ...
        if (local)
            return { ...local, checked_locally_only: true };
