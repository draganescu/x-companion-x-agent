 ...
 */
import { z } from 'zod';
import { DesignSpecIRSchema, checkWithZod } from '../schemas.js';
...
/* ------------------------------------------------------------------ checks */
function collectRegionIds(regions, into) {
    for (const r of regions ?? []) {
        into.add(r.id);
        if (r.children?.length)
            collectRegionIds(r.children, into);
    }
}
function checkBoxes(regions, basePath, out) {
    regions?.forEach((parent, i) => {
        ...
        parent.children?.forEach((child, j) => {
            ...
            const breaches = [];
            if (cLeft < pLeft)
                breaches.push(`left ${cLeft} < ${round(pLeft)}`);
            if (cTop < pTop)
                breaches.push(`top ${cTop} < ${round(pTop)}`);
            if (cRight > pRight)
                breaches.push(`right ${round(cRight)} > ${round(pRight)}`);
            if (cBottom > pBottom)
                breaches.push(`bottom ${round(cBottom)} > ${round(pBottom)}`);
            if (breaches.length) {
                out.push({
                    code: 'E_BOX_OVERLAP',
                    path: `${childPath}/box`,
                    message: `Region "${child.id}" escapes its parent "${parent.id}" (2% slack applied): ${breaches.join(', ')}.`,
                    fix_hint: 'Re-measure the child box in source pixels; a child box must sit inside its parent box. Nested regions are geometry, not z-order.',
                });
            }
        });
        if (parent.children?.length)
            checkBoxes(parent.children, `${parentPath}/children`, out);
    });
}
function round(n) {
...
export function quantizableValues(spec) {
    const t = spec.tokens_candidates;
    const out = [];
    t.palette.forEach((p, i) => out.push({ path: `/tokens_candidates/palette/${i}/color`, value: p.color, label: `palette "${p.slug}"` }));
    t.spacing.steps.forEach((s, i) => out.push({ path: `/tokens_candidates/spacing/steps/${i}/size`, value: s.size, label: `spacing step "${s.slug}"` }));
    t.typography.sizes.forEach((s, i) => out.push({ path: `/tokens_candidates/typography/sizes/${i}/size`, value: s.size, label: `font size "${s.slug}"` }));
    ...
    if (schemaIssues.length) {
        return {
            ...
            diagnostics: schemaIssues.map((i) => ({
                ...
                path: i.path,
                message: i.message,
                fix_hint: 'Fix the spec against schemas/design-spec.schema.json. Every object in the spec is additionalProperties:false — an unexpected key is an error, not a hint.',
            })),
        };
    }
    ...
    for (const v of quantizableValues(spec)) {
        if (!snapped.has(String(v.value))) {
            diagnostics.push({
                ...
                fix_hint: 'Every token value must be a deliberate snap of an observed measurement. Add {observed, snapped_to, delta} to tokens_candidates.quantization_log so the delta is reviewable.',
            });
        }
    }
    ...
    spec.regions.forEach((r, i) => {
        if (!r.responsive_assumptions || r.responsive_assumptions.length === 0) {
            diagnostics.push({
                ...
                fix_hint: 'State at least one breakpoint behaviour per top-level region. Mark anything you did not observe in the source with confidence:"synthesized" so a human can veto it.',
            });
        }
    });
