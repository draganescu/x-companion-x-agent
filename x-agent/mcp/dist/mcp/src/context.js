 ...
 * ============================================================================
 */
import { CompanionClient, ManifestCache, stderrLogger, } from './companion.js';
import { resolveConfig, configIdentity } from './config.js';
import { Profiler } from './profiler.js';
let sessionProvider;
let factoryProvider;
export function registerSessionProvider(p) {
    sessionProvider = p;
}
export function registerFactoryProvider(p) {
    factoryProvider = p;
}
export function hasSessionProvider() {
    return sessionProvider !== undefined;
}
export function hasFactoryProvider() {
...
export class Runtime {
    ...
    setOverrides(args) {
        ...
        if (args.url)
            clean.url = args.url;
        if (args.user)
            clean.user = args.user;
        if (args.app_password)
            clean.app_password = args.app_password;
    ...
    ctx(args = {}, opts = {}) {
        ...
        const resolveOpts = {};
        if (this.opts.cwd)
            resolveOpts.cwd = this.opts.cwd;
        if (this.opts.env)
            resolveOpts.env = this.opts.env;
        const config = resolveConfig(merged, resolveOpts);
        const id = configIdentity(config);
        if (!this.client || this.identity !== id) {
            if (this.identity !== id)
                void this.disposeInstances();
            const clientOpts = { config, logger: this.logger };
            if (this.opts.fetchImpl)
                clientOpts.fetchImpl = this.opts.fetchImpl;
    ...
    stubCtx(cause) {
        ...
        const c = { logger: this.logger, runtime: this };
    ...
    async disposeInstances() {
        ...
        if (this.sessionInstance !== undefined) {
            try {
                ...
                this.logger.warn(`session dispose failed: ${e.message}`);
            }
        ...
        if (this.factoryInstance !== undefined) {
            try {
                ...
                this.logger.warn(`factory dispose failed: ${e.message}`);
            }
...
export const getSession = (ctx) => ctx.runtime.getSession(ctx);
export const getFactory = (ctx) => ctx.runtime.getFactory(ctx);
