...
export function isXError(e) {
    return e instanceof XError || (typeof e === 'object' && e !== null && e.name === 'XError');
}
...
export function redact(input) {
    ...
    for (const s of SECRETS) {
        ...
        if (compact.length >= 8 && compact !== s)
            out = out.split(compact).join('***');
    }
...
/* ------------------------------------------------------- common constructors */
export const errInvalidInput = (message, hint = 'Fix the tool arguments and call again.', extra = {}) => new XError('invalid_input', message, hint, extra);
export const errNotImplemented = (tool) => new XError('not_implemented', `Tool "${tool}" is declared but its handler module is not present in this build.`, 'This tool ships in the session/oracle/factory track (src/tools/{compile,verify,screenshot,blockScaffold,blockBuildTest,blockInstall}.ts). Install a build that includes it.', { tool });
export const errPostureForbidden = (route) => new XError('posture_forbidden', `The connected instance has posture "production"; ${route} is an extend-tier route and is refused by design.`, 'clone to sandbox via wp_snapshot then apply there', { route });
export const errEpochMismatch = (expected, actual) => new XError('epoch_mismatch', `The instance fingerprint moved and stayed moved after one refresh+retry (expected ${expected}, instance reports ${actual}).`, 'Call wp_manifest({refresh:true}), regenerate the tree with the new epoch, and retry.', { expected_fingerprint: expected, server_fingerprint: actual });
