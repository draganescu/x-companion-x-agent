...
import { z } from 'zod';
import { TOOLS, loadExternalHandlers, findTool, isUnimplemented } from './registry.js';
import { Runtime } from './context.js';
import { toEnvelope, XError, redact } from './errors.js';
import { stderrLogger } from './companion.js';
 ...
 */
export async function callTool(name, args, runtime) {
...
async function callToolInner(name, args, runtime) {
    const tool = findTool(name);
    if (!tool) {
        return errorResult(new XError('invalid_input', `Unknown tool "${name}".`, `Known tools: ${TOOLS.map((t) => t.name).join(', ')}`));
    }
    try {
        ...
        if (!parsed.success) {
            const detail = parsed.error.issues
                ...
                .join('; ');
            throw new XError('invalid_input', `Arguments for ${name} failed validation: ${detail}`, 'Re-read the tool input schema and call again with corrected arguments.', { tool: name });
        }
        ...
        if (!out.success) {
            const detail = out.error.issues
                ...
                .map((i) => `${i.path.length ? i.path.join('.') : '(root)'}: ${i.message}`)
                .join('; ');
            throw new XError('internal', `Tool ${name} produced output that does not match its own output schema: ${detail}`, 'This is an agent-side bug; report it with the tool name and arguments.', { tool: name });
        }
        return {
...
function connectionArgsFrom(args) {
    ...
    if (typeof a.url === 'string')
        out.url = a.url;
    if (typeof a.user === 'string')
        out.user = a.user;
    if (typeof a.app_password === 'string')
        out.app_password = a.app_password;
    return out;
}
export async function createServer(opts = {}) {
    ...
    if (report.missing.length) {
        logger.info(`${report.missing.length} tool handler(s) not present in this build; they list normally and return {code:"not_implemented"} when called: ${report.missing.join(', ')}`);
    }
    for (const f of report.failed)
        logger.warn(`tool module ${f.module} failed to load: ${redact(f.error)}`);
    const server = new Server({ name: SERVER_NAME, version: SERVER_VERSION }, { capabilities: { tools: {} } });
    server.setRequestHandler(ListToolsRequestSchema, async () => ({ tools: TOOLS.map(describeTool) }));
    server.setRequestHandler(CallToolRequestSchema, async (req) => {
        const { name, arguments: args } = req.params;
        return (await callTool(name, args, runtime));
    });
...
const invokedDirectly = (() => {
    ...
    if (!entry)
        return false;
    const normalise = (p) => p.split(/[/\\]/).pop() ?? '';
    return normalise(new URL(import.meta.url).pathname) === normalise(entry);
})();
if (invokedDirectly) {
    main().catch((e) => {
        process.stderr.write(`[x-agent fatal] ${redact(e.message)}\n`);
        process.exit(1);
    });
}
