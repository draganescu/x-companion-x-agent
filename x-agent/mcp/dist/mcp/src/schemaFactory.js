...
import { SMOKE_RUNNER_SOURCE, TAG, freePort, loadAdmZip, phpJson, resolvePlaygroundCli, run, } from './factory.js';
...
export function policyScan(dir) {
    const violations = [];
    for (const file of fs.readdirSync(dir)) {
        ...
        lines.forEach((line, i) => {
            for (const { re, what } of FORBIDDEN_TOKENS) {
                if (re.test(line))
                    violations.push({ file, what, line: i + 1 });
            }
        });
    }
    return violations;
...
/* Code generation                                                            */
/* ========================================================================== */
function assertSchemaSlug(slug, what) {
...
function mainPluginPhp(input, version) {
    ...
    for (const cpt of input.post_types) {
        ...
        if ((cpt.meta ?? []).length && !supports.includes('custom-fields'))
            supports.push('custom-fields');
        ...
        for (const meta of cpt.meta ?? []) {
            ...
            if (meta.default !== undefined)
                lines.push(`\t\t'default'      => ${phpArray(meta.default)},`);
...
export function packageMeta(input, version) {
    const metaKeys = [];
    for (const cpt of input.post_types) {
        for (const meta of cpt.meta ?? [])
            metaKeys.push({ post_type: cpt.slug.replace(/-/g, '_'), key: meta.key });
    }
    return {
        slug: input.slug,
        version,
...
/* Scaffold                                                                   */
/* ========================================================================== */
export function schemaScaffold(input) {
...
function routesProbePhp(meta, slug) {
    return `<?php
require_once '/wordpress/wp-load.php';
...
function uninstallProbePhp(meta, pluginSlug, pluginFile) {
    return `<?php
require_once '/wordpress/wp-load.php';
wp_set_current_user( 1 );
...
function postUninstallProbePhp(meta) {
    return `<?php
require_once '/wordpress/wp-load.php';
...
/* Build test                                                                 */
/* ========================================================================== */
function readTaggedJson(outcome) {
    if (!outcome)
        return null;
...
export async function schemaBuildTest(dir, opts = {}) {
    ...
    const phpError = [raw.probes?.model, raw.probes?.routes, raw.probes?.uninstall, raw.probes?.post_uninstall]
        .filter((p) => Boolean(p?.threw))
        .map((p) => p.error_text)
        .join(' | ') || undefined;
    ...
    const failures = [];
    for (const [slug, ok] of Object.entries(smoke.types_registered))
        if (!ok)
            failures.push(`post type "${slug}" missing from /wp/v2/types`);
    for (const [key, ok] of Object.entries(smoke.meta_in_rest))
        if (!ok)
            failures.push(`meta "${key}" not REST-visible (show_in_rest with a schema is policy)`);
    for (const [slug, ok] of Object.entries(smoke.taxonomies_registered))
        if (!ok)
            failures.push(`taxonomy "${slug}" not registered`);
    for (const [source, ok] of Object.entries(smoke.bindings_registered))
        if (!ok)
            failures.push(`binding source "${source}" not registered`);
    for (const r of smoke.routes)
        if (!r.ok)
            failures.push(`route ${r.method} ${r.path} answered ${r.status}${r.unauth_status !== undefined ? ` / unauth ${r.unauth_status}` : ''}`);
    if (!smoke.uninstall_clean)
        failures.push(`uninstall leaves registrations behind: ${JSON.stringify(postUninstall?.leftover ?? 'unknown')}`);
    if (phpError)
        failures.push(`php error: ${phpError}`);
    if (failures.length) {
        return {
            built: false,
            smoke,
