 ...
 */
export const THEME_SEAM_RESET = '.wp-site-blocks > * + * { margin-block-start: 0; }\n' +
...
/* ========================================================================== */
const THEME_PORT_RANGE = [9480, 9489];
const DEFAULT_THEME_SMOKE_TIMEOUT_MS = 5 * 60 * 1000;
function themePortRange() {
...
export async function freeThemePort(preferred) {
...
export function themeProbePhp(slug, rail) {
...
export function publishPhysicsPagePhp() {
    return `<?php
require_once '/wordpress/wp-load.php';
wp_set_current_user( 1 );
...
function probeJson(outcome) {
    if (!outcome)
        return {};
    try {
        return JSON.parse((outcome.text.split(TAG)[1] ?? '{}'));
    }
    catch {
        return {};
    }
}
const tailText = (s, n = 4000) => (s.length > n ? s.slice(-n) : s);
/**
 ...
 */
export async function buildAndTestTheme(input) {
    ...
    if (!fs.existsSync(path.join(themeDir, 'style.css')) || !fs.existsSync(path.join(themeDir, 'templates', 'index.html'))) {
        throw errInvalidInput(`${themeDir} is not a scaffolded block theme (style.css + templates/index.html required).`, 'Point dir at a wp_theme_scaffold output.');
    }
    ...
    if (rail)
        declared.railWidth = themeJson.settings?.custom?.railWidth ?? THEME_RAIL_WIDTH;
    ...
    const roster = probeJson(raw.probes?.roster);
    ...
    if (rail)
        smoke.rail_area_registered = (roster.areas ?? []).includes('rail');
    ...
    if (smoke.php_error)
        return fail(`PHP errors during the theme probes: ${tailText(smoke.php_error, 600)}`);
    if (!smoke.activated)
        return fail(`The theme did not activate: active stylesheet is "${smoke.stylesheet}", expected "${slug}".`);
    if (smoke.templates_missing.length > 0) {
        const missing = smoke.templates_missing.join(', ');
        return fail(smoke.templates_missing.includes('page-no-title')
    ...
    if (raw.physics?.error)
        return fail(`The physics pass crashed: ${tailText(raw.physics.error, 600)}`);
    const m = raw.physics?.measured;
    if (!m)
        return fail('The physics pass returned no measurements.');
    ...
    if (m.root_gap_px > 1)
        return fail(`Root block-gap seams measured ${m.root_gap_px.toFixed(1)}px — the flush-band physics did not hold.`);
    if (!close(m.padding_top_px, m.padding_top_declared_px, 1) || !close(m.padding_bottom_px, m.padding_bottom_declared_px, 1)) {
        return fail(`Root padding measured ${m.padding_top_px}px/${m.padding_bottom_px}px (top/bottom), declared resolves to ${m.padding_top_declared_px}px/${m.padding_bottom_declared_px}px.`);
    }
