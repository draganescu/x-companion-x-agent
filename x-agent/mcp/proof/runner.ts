 ...
 *
 * This is not a unit-test runner. Its job is to demonstrate, against a REAL
 * WordPress instance, that x-companion and x-agent are actually working
 * together — and to write down what it observed so a human can check the claim
 * without re-running anything.
 *
 * Design notes:
 *  - Scenarios are grouped by the instance they need, so we boot each Playground
 *    instance once instead of once per scenario (a boot is ~10s).
 *  - Every scenario records STEPS, each with a real observed value. A scenario
 *    that passes with no recorded observations is treated as suspicious, not as
 *    a pass, because "it didn't throw" is not evidence.
 *  - The agent side is driven through the REAL MCP tool entrypoint (callTool),
 *    not through internal helpers. If the tools work here, they work in Claude.
 */
...
export class Recorder {
  ...
  eq(what: string, actual: unknown, expected: unknown): void {
    ...
    this.steps.push({
      what,
      observed: ok ? render(actual) : `${render(actual)}   (expected ${render(expected)})`,
      ok,
    });
...
export interface Scenario {
  ...
  proves: string;
  needs: InstanceKey;
  /** Skip with a reason instead of failing, when a precondition is legitimately absent. */
  ...
  for (const s of scenarios) {
    ...
    try {
      ...
      if (out && typeof out === 'object' && 'skipped' in out) {
        results.push({ ...meta(s), status: 'skip', detail: out.skipped, steps: t.steps, ms: Date.now() - started });
        process.stdout.write(`   SKIP  ${out.skipped}\n`);
      } else {
        results.push({ ...meta(s), status: 'pass', steps: t.steps, ms: Date.now() - started });
        for (const st of t.steps) process.stdout.write(`   ✓ ${st.what}  →  ${st.observed}\n`);
        process.stdout.write(`   PASS  (${t.steps.length} observations, ${Date.now() - started}ms)\n`);
      }
    } catch (e: any) {
...
function meta(s: Scenario) {
  return { id: s.id, title: s.title, proves: s.proves, needs: s.needs };
}
...
export function writeReport(results: Result[], env: Record<string, string>): string {
  ...
  L.push('for what each scenario is meant to establish and why.');
  L.push('');
  L.push('| | |');
  L.push('|---|---|');
  ...
  for (const r of results) {
    ...
    L.push(`| [${r.id}](#${r.id.toLowerCase()}) ${r.title} | ${badge} | ${r.proves} |`);
  }
  L.push('');
  L.push('---');
  L.push('');
  for (const r of results) {
    ...
    if (r.steps.length) {
      ...
      for (const s of r.steps) {
        L.push(`| ${s.ok ? '✓' : '✗'} ${escapePipes(s.what)} | \`${escapePipes(s.observed)}\` |`);
      }
      L.push('');
    }
    if (r.detail) {
      L.push('```');
      L.push(r.detail.slice(0, 2000));
      L.push('```');
      L.push('');
    }
  }
