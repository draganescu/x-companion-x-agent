 ...
 * drives. The model is never trusted with seamlessness or alpha — code
 * guarantees both:
 *
 *   pattern  -> mirror-tiled 2x2 into a reflected sheet (seams impossible by
 *               construction), kept lossless as PNG so the guarantee survives
 *               encoding
 *   spot     -> chroma-key knockout of the recorded key color to a real alpha
 *               PNG
 *   field / frieze / canvas -> recompressed JPEG at class-appropriate quality
 *
 * Every class also has its luminance range measured while the pixels are in
 * hand — the ink menus for anything sitting on the asset are computed from it.
 * No randomness anywhere: identical input bytes produce identical output bytes.
 */
...
export interface ProcessResult {
  bytes: Buffer;
  mime_type: string;
  post_processing: 'mirror-tile' | 'chroma-key' | 'recompress';
  /** Measured relative-luminance range (0..1) over the visible pixels. */
  lum_min: number;
  lum_max: number;
}
...
const VEILED_CLASSES = new Set<SurfaceClass>(['field', 'pattern', 'canvas']);
...
export async function processAsset(
  ...
  input: Buffer,
  inputMime: string,
  opts: { class: SurfaceClass; key_hex?: string; veil?: { hex: string; alpha: number } | null },
  ...
  try {
    ...
    const raw = (await page.evaluate(async (p: typeof payload) => {
      try {
        ...
        if (p.cls === 'pattern') {
          ...
          g.drawImage(img, 0, 0);
          g.save(); g.scale(-1, 1); g.drawImage(img, -2 * w, 0); g.restore();
          g.save(); g.scale(1, -1); g.drawImage(img, 0, -2 * h); g.restore();
          g.save(); g.scale(-1, -1); g.drawImage(img, -2 * w, -2 * h); g.restore();
        } else {
          ...
          if (p.cls === 'spot' && p.keyHex) {
            ...
            for (let i = 0; i < px.length; i += 4) {
              ...
              const dg = px[i + 1]! - kg;
              const db = px[i + 2]! - kb;
              if (Math.sqrt(dr * dr + dg * dg + db * db) <= p.tolerance) px[i + 3] = 0;
            }
    ...
    }, payload)) as InPageResult & { _mode?: string };
