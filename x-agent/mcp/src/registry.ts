 ...
 * -----------------------------
 * Six of the sixteen tools (wp_compile, wp_verify, wp_screenshot,
 ...
 *      all sixteen with full schemas whether or not the other track's modules
 ...
 * track can add tools beyond the sixteen without touching this file.
 *
...
export interface ToolDef {
  ...
  handler: (input: any, ctx: Ctx) => Promise<any>;
  /** True when the tool needs no resolved connection config. */
  local?: boolean;
}
...
const EXTERNAL_TOOLS: Omit<ToolDef, 'handler'>[] = [
  {
    name: 'wp_compile',
...
function placeholder(def: Omit<ToolDef, 'handler'>): ToolDef {
  return {
    ...def,
    handler: async () => {
...
export function unimplementedToolNames(): string[] {
...
export function isUnimplemented(name: string): boolean {
  return PLACEHOLDERS.has(name);
}

/* -------------------------------------------- dynamic failure-tolerant load */

function looksLikeToolDef(v: unknown): v is ToolDef {
  if (!v || typeof v !== 'object') return false;
  const t = v as Partial<ToolDef>;
  return typeof t.name === 'string' && typeof t.handler === 'function' && Boolean(t.inputSchema);
}

function collectToolDefs(mod: Record<string, unknown>): ToolDef[] {
  const out: ToolDef[] = [];
  for (const value of Object.values(mod)) {
    if (looksLikeToolDef(value)) out.push(value);
    else if (Array.isArray(value)) for (const v of value) if (looksLikeToolDef(v)) out.push(v);
  }
  return out;
}
 ...
 */
export async function loadExternalHandlers(opts: { logger?: Logger; force?: boolean } = {}): Promise<LoadReport> {
  const report: LoadReport = { loaded: [], missing: [], failed: [], added: [] };
  if (loaded && !opts.force) return report;
  loaded = true;
  ...
  for (const [base, expectedName] of Object.entries(EXTERNAL_MODULES)) {
    ...
    try {
      ...
      report.failed.push({ module: base, error: (e as Error).message });
      opts.logger?.warn(`tool module ${base} failed to load, keeping not_implemented placeholder: ${(e as Error).message}`);
      continue;
    }
