...
export const wpSchemaInstall = defineTool({
  ...
  description:
    'POST /schema/install with the zip from wp_schema_build_test. The package registers its post types, taxonomies, meta, bindings and routes on init from then on; the returned fingerprint is the NEW epoch — the manifest data_model will list the registrations with source "agent", and every subsequent tree must carry the new fingerprint. EXTEND TIER: refused with {code:"posture_forbidden"} on production posture — snapshot to a sandbox and install there.',
  ...
  handler: async (input: unknown, ctx: Ctx) => {
    ...
    if (!fs.existsSync(zipPath) || !fs.statSync(zipPath).isFile()) {
      throw errInvalidInput(
        ...
        'Pass the zip_path returned by wp_schema_build_test. If it returned no zip_path, the package failed its gate and must be fixed first.',
    ...
    if (fp.posture === 'production') throw errPostureForbidden('/schema/install');
    ...
    try {
      ...
      live.logger.warn(`installed schema ${res.installed.slug} but could not refresh the manifest: ${(e as Error).message}`);
    }
    ...
    if (live.session !== undefined) {
      try {
        ...
        live.logger.warn(`installed schema ${res.installed.slug} but the harness session did not reload: ${(e as Error).message}`);
      }
    }
