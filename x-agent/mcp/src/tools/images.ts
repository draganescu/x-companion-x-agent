...
import { findPlaceholders, applyImage, type PlaceholderRef } from '../images/scan.js';

const MANIFEST_FILENAME = 'images-manifest.json';

interface ManifestImage extends PlaceholderRef {
  file: string;
  prompt: string;
  mime_type: string;
  bytes: number;
...
export const wpImagesGenerate = defineTool({
  ...
  handler: async (input: unknown, ctx: Ctx) => {
    ...
    const manifest: Manifest = { post_id: args.post_id, rest_base: restBase, model, images };
    if (args.style) manifest.style = args.style;
...
export const wpImagesApply = defineTool({
  ...
  handler: async (input: unknown, ctx: Ctx) => {
    ...
    if (manifest.post_id !== args.post_id) {
      throw errInvalidInput(
        `The manifest at ${manifestPath} is for post ${manifest.post_id}, not ${args.post_id}.`,
        'Point manifest_path at the right run of wp_images_generate.',
      );
    }
