...
import {
  ...
  buildSurfacePrompt,
  fixturePathFor,
} from '../images/gemini.js';
import { scanRefs, applyImage, applySurface, stripSurface, type ContentRef } from '../images/scan.js';
import {
  MANIFEST_FILENAME,
  emptyManifest,
  loadManifest,
...
async function birthImage(
  gemini: GeminiImages | null,
  prompt: string,
  aspect: string,
): Promise<{ data: Buffer; mimeType: string; ms: number }> {
  ...
  if (fixtureDir && fixtureDir.trim() !== '') {
    ...
    for (const [file, mime] of [[candidates.jpg, 'image/jpeg'], [candidates.png, 'image/png']] as const) {
      if (fs.existsSync(file)) return { data: fs.readFileSync(file), mimeType: mime, ms: 0 };
    }
    throw new XError('invalid_input', `No image fixture for this prompt (looked for ${path.basename(candidates.jpg)}).`,
      'Capture fixtures with a real run first, or unset X_AGENT_IMAGE_FIXTURES.');
  }
  if (!gemini) {
    throw new XError(
      'invalid_input',
      'No Gemini API key configured, and generating images needs one.',
      'Add "gemini_api_key" to .x-agent.json (or set GEMINI_API_KEY). Keys: https://aistudio.google.com/apikey',
    );
  }
...
export const wpImagesGenerate = defineTool({
  ...
  handler: async (input: unknown, ctx: Ctx) => {
    ...
    let surfaceRefs: ReturnType<typeof scanRefs>['surfaces'] = [];
    ...
    const dictionary: SurfaceCallSpec[] = (args.surfaces ?? []).map((s) => ({ ...s }));
    const surfacePathsById = new Map<string, string[]>();
    ...
    const images: ManifestContentEntry[] = [];
    const CONCURRENCY = 3;
      ...
      const results = await Promise.allSettled(
        batch.map(async (ref, j) => {
          ...
          return {
            ...
            ms: result.ms,
          } satisfies ManifestContentEntry;
        }),
      );
      results.forEach((r, j) => {
        if (r.status === 'fulfilled') images.push(r.value);
        else failures.push(`${batch[j]!.path}: ${(r.reason as Error)?.message ?? String(r.reason)}`);
      });
    }
    ...
    for (const spec of surfacePlan.generate) {
      try {
        ...
        for (const attemptPrompt of [prompt, `${prompt} ${SURFACE_RETRY_SUFFIX}`]) {
          ...
          if (ornament && stationOpts.alpha_scale) {
            ...
            if (trimmed !== null && trimmed < stationOpts.alpha_scale) {
              ...
              if (processed.lum_max - processed.lum_min <= bound) break;
            }
          }
        ...
        const entry: ManifestSurfaceEntry = {
          kind: 'surface',
          asset_id: spec.id,
          class: spec.class,
          file,
        ...
        if (spec.intensity) entry.intensity = spec.intensity;
        surfacesOut.push(entry);
      } catch (e) {
        failures.push(`surface ${spec.id}: ${(e as Error)?.message ?? String(e)}`);
      }
    }
  ...
  handler: async (input: unknown, ctx: Ctx) => {
    ...
    if (!fs.existsSync(manifestPath)) {
      throw errInvalidInput(
        ...
        'Run wp_images_generate first, or pass manifest_path to the file it reported.',
      );
    }
    ...
    if (content.length === 0 && surfaceWork.length === 0) {
      throw errInvalidInput(
        `The manifest at ${manifestPath} has no entries for post ${args.post_id}.`,
        'Run wp_images_generate against this post first.',
      );
    }
    ...
    if (args.strip_surfaces && args.strip_surfaces.length > 0) {
      ...
      for (const assetId of args.strip_surfaces) {
        ...
        for (const target of entry.targets.filter((t) => t.post_id === args.post_id)) {
          if (stripSurface(tree, target, entry.media_url)) stripped += 1;
          else stripSkipped.push(`${target.path}: does not carry our ${assetId} application — not touched`);
        }
      }
      if (stripped === 0) {
      ...
      const compiled = await session.compile(tree as BlockNode[]);
    ...
    const uploaded: { id: number; source_url: string }[] = [];
    ...
    const surfaceOutcomes: { asset_id: string; media_id?: number; media_url?: string; applied: number; refused: number }[] = [];
    for (const { entry, targets } of surfaceWork) {
      const outcome = { asset_id: entry.asset_id, applied: 0, refused: 0 } as (typeof surfaceOutcomes)[number];
    ...
    if (manifestDirty) saveManifest(manifestPath, manifest);
    ...
    await live.manifestCache.get({ fingerprintMinIntervalMs: 0 });
    const result = await session.compile(tree as BlockNode[]);
    if (!result.all_valid) {
