...
import { BlockNodeSchema, type BlockNode } from '../schemas.js';
  ...
  handler: async (input: unknown, ctx: Ctx) => {
    ...
    if (!Array.isArray(raw.blocks)) {
      throw errInvalidInput(
        'wp_compile needs a TreeIR: {version: 1, epoch: "<fingerprint>", blocks: BlockNode[]}. `blocks` was ' +
          (raw.blocks === undefined ? 'missing' : typeof raw.blocks) + '.',
        'Generate a tree (never markup) and pass it whole. wp_validate will tell you if the tree itself is wrong.',
      );
    }
    ...
    if (!parsed.success) {
      const detail = parsed.error.issues
        ...
        .join('; ');
      throw errInvalidInput(
        `The tree does not satisfy tree-ir.schema.json: ${detail}`,
        'Run wp_validate first — it reports E_TREE_SCHEMA with an RFC 6901 pointer. Remember a tree never contains innerHTML.',
      );
    }
    const blocks = parsed.data as BlockNode[];
