...
const InputSchema = z.looseObject({
  spec: ThemeSpecSchema.describe(
    'The ThemeSpec — identity, skeleton (stacked|split|rail), measure, physics, presets. Validated against contract/schemas/theme-spec.schema.json.',
  ),
...
export const wpThemeScaffold = defineTool({
  ...
  description:
    'Compiles a ThemeSpec into a complete, named block theme directory — deterministically. The model authors the spec; this tool owns every byte on disk. The theme ships structure only: no patterns, no block styles, no fonts. Next step is always wp_theme_build_test.',
  inputSchema: InputSchema,
  outputSchema: OutputSchema,
  local: true,
  handler: async (input: unknown, _ctx: Ctx) => {
    const args = InputSchema.parse(input ?? {});
    const opts: Parameters<typeof scaffoldTheme>[1] = {};
    if (args.dir) opts.dir = args.dir;
