...
const InputSchema = z.object({
  ...
  styles: DesignTokensSchema.shape.styles.describe(
    'x-surfaces: {background: {backgroundImage: {url, id?}, backgroundSize?, backgroundRepeat?, backgroundPosition?, backgroundAttachment?}} — the page canvas, shipped through global styles (WP >= 6.6; probe manifest features.global_styles_background first). Admin-undoable in the Styles UI. Rejections come back itemized in background_rejected.',
  ),
  dry_run: z
    .boolean()
    .optional()
...
export const wpTokensApply = defineTool({
  ...
  description:
    'POST /theme/tokens — writes the design system into the instance (user-origin global styles, plus any suite adapters). EXTEND TIER: refused with {code:"posture_forbidden"} on a production-posture instance; snapshot to a sandbox and apply there instead. Always returns the local theme.json settings preview and a diff against the instance tokens, so dry_run:true is a free rehearsal.',
  ...
  handler: async (input: unknown, ctx: Ctx) => {
    ...
    if (manifest.posture === 'production') throw errPostureForbidden('/theme/tokens');
