...
export interface BuildTestInput {
  ...
  /** Hard ceiling for the per-script syntax checks, ms. */
  timeout_ms?: number;
...
export function renderEditorGlobals(attrs: ScaffoldAttribute[]): string {
  ...
  if (kinds.has('structured')) lines.push(`\tconst { useState } = wp.element;`);
  return lines.join('\n');
}
...
export const SCAFFOLD_FILES = ['block.json', 'render.php', 'edit.js', 'edit.asset.php'] as const;
...
export async function syntaxGate(
...
): Promise<{ ok: boolean; log: string; ms: number }> {
  const started = Date.now();
  const meta = readBlockMetadata(dir);
  const raw = meta.raw as Record<string, unknown>;
  const refOf = (v: unknown): string | null =>
    typeof v === 'string' && v.startsWith('file:') ? v.slice('file:'.length).replace(/^\.\//, '') : null;
  const entries: Array<{ rel: string; module: boolean }> = [];
  for (const key of ['editorScript', 'script', 'viewScript']) {
    const rel = refOf(raw[key]);
    if (rel && rel.endsWith('.js')) entries.push({ rel, module: false });
  }
  const mod = refOf(raw.viewScriptModule);
  if (mod && mod.endsWith('.js')) entries.push({ rel: mod, module: true });
  ...
  for (const { rel, module } of entries) {
    ...
    if (module) fs.copyFileSync(abs, target);
    try {
      ...
      if (module) fs.rmSync(target, { force: true });
    }
  }
