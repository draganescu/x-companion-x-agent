...
export type ConfigFieldSource = 'arguments' | 'file' | 'env' | 'missing';

export interface XConfig {
  /** Normalised site URL, no trailing slash, no userinfo. */
  site_url: string;
  user: string;
  /** Never log this. Never emit this. */
  app_password: string;
...
export interface ResolveOptions {
  ...
  /** Skip the https guard (used by nothing in prod; tests assert the guard itself). */
 ...
 */
export function isInsecureAllowedHost(hostname: string): boolean {
  const h = hostname.toLowerCase().replace(/^\[|\]$/g, '');
  if (LOCAL_HOSTNAMES.has(h) || LOCAL_HOSTNAMES.has(hostname.toLowerCase())) return true;
  if (h.endsWith('.localhost')) return true;
  if (h.split('.').includes('playground')) return true;
  return false;
}
...
export function assertTransportAllowed(rawUrl: string): URL {
  ...
  try {
    ...
    throw new XError(
      'invalid_input',
      `"${rawUrl}" is not a valid absolute URL.`,
      'Pass the full site URL including scheme, e.g. https://example.com',
    );
  }
  if (u.protocol === 'https:') return u;
  if (u.protocol === 'http:') {
    if (isInsecureAllowedHost(u.hostname)) return u;
    throw new XError(
      'https_required',
      `Refusing to send an application password over plain http:// to "${u.hostname}".`,
      'Use https:// for the site URL. Plain http is only allowed for localhost, 127.0.0.1, [::1], *.localhost and playground hosts.',
      { host: u.hostname },
    );
  }
  throw new XError(
    ...
    'The site URL must be http:// (local only) or https://.',
  );
}

/** Strip trailing slashes and any embedded userinfo. */
export function normaliseSiteUrl(u: URL): string {
...
export function readConfigFile(cwd: string): { data: ConfigFileShape; file?: string } {
  ...
  try {
    ...
    throw new XError(
      'invalid_input',
      `${CONFIG_FILENAME} in ${cwd} is not valid JSON: ${(e as Error).message}`,
      `Fix the JSON in ${file} or delete it and use the X_WP_* environment variables.`,
    );
  }
  if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
    throw new XError('invalid_input', `${CONFIG_FILENAME} must contain a JSON object.`, `Fix ${file}.`);
  }
  return { data: parsed as ConfigFileShape, file };
}
...
): { value?: string; source: ConfigFieldSource } {
  if (typeof args === 'string' && args.trim() !== '') return { value: args.trim(), source: 'arguments' };
  if (typeof file === 'string' && file.trim() !== '') return { value: file.trim(), source: 'file' };
  if (typeof env === 'string' && env.trim() !== '') return { value: env.trim(), source: 'env' };
  return { source: 'missing' };
}

export function resolveConfig(args: ConnectionArgs = {}, opts: ResolveOptions = {}): XConfig {
  ...
  if (missing.length) {
    throw new XError(
      'invalid_input',
      `Missing connection config: ${missing.join(', ')}.`,
      `Pass {url, user, app_password} to wp_connect, or create ${CONFIG_FILENAME} in the working directory, or set X_WP_URL / X_WP_USER / X_WP_APP_PASSWORD.`,
      { missing },
    );
  }
...
export function configIdentity(cfg: XConfig): string {
  return `${cfg.site_url}|${cfg.user}`;
}

/** Safe-to-log view of the config. */
export function describeConfig(cfg: XConfig): Record<string, unknown> {
  return {
