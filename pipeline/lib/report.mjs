...
const PREDICTED = { brief: () => 1, theme: (b) => b.T ?? 0, tokens: () => 1, tree: (b) => b.S + (b.F ?? 0), block: (b) => b.B, schema: (b) => b.P, image: (b) => b.I, repair: () => 0 };
...
export function writeReport(runDir, { state, budget, ledger }) {
    ...
    if (state.budget) {
        ...
        lines.push(`Ceiling **${b.ceiling}** (base ${b.base}: 1 brief${b.T ? ` + T=${b.T} theme` : ''} + 1 tokens + F=${b.F ?? 0} furniture + S=${b.S} + B=${b.B} + P=${b.P}; 2x + I=${b.I}). Spent **${budget.spent}**.`, '');
